...
def main(DATASET, WINDOW_SIZE, VERSIONS_AHEAD,EXP,config):
    ...
    def create_regressor(reg_type, X, Y, project, versions_ahead, config):
        ...
        def lstm_model(optimizer='adam', activation="relu", neurons = 50,learn_rate = 0.01, dropout_rate=0.25, layers = 1):
            ...
            if layers == 1:
                ...
                model.compile(loss='mean_squared_error', optimizer=opt)
                return model
        def lstm_model2(optimizer='adam', activation="relu", neurons = 50,learn_rate = 0.01, dropout_rate=0.1, layers = 2):
            # LSTM layer expects inputs to have shape of (batch_size, timesteps, input_dim).
            # In keras you need to pass (timesteps, input_dim) for input_shape argument.
            K.clear_session()
            opt = optimizers.Adam(learning_rate=learn_rate)
        ...
        def lstm_model3(optimizer='adam', activation="relu", neurons = 200,learn_rate = 0.01, dropout_rate=0.1, layers = 2):
            # LSTM layer expects inputs to have shape of (batch_size, timesteps, input_dim).
            # In keras you need to pass (timesteps, input_dim) for input_shape argument.
            K.clear_session()
            opt = optimizers.Adam(learning_rate=learn_rate)
        ...
        if reg_type == 'SGDRegression':
            ...
            if config == 3:
               regressor = KerasRegressor(build_fn=lstm_model3, epochs=500, batch_size=10, verbose=False)
            # pipeline = Pipeline([('scaler', scaler), ('regressor', regressor)])
            pipeline = Pipeline([('regressor', regressor)])
