...
def main(DATASET, WINDOW_SIZE, VERSIONS_AHEAD,EXP,comb):
    ...
    def create_regressor(reg_type, X, Y, project, versions_ahead):
        ...
        def lstm_model(optimizer='adam', activation="relu", neurons = 50,learn_rate = 0.1, dropout_rate=0.2, layers = 1):
            ...
            if layers == 1:
                model1 = Sequential()
                model1.add(LSTM(neurons, input_shape=(9, 1), kernel_initializer='normal', activation=activation))
                ...
                model1 = Sequential()
                model1.add(LSTM(neurons, input_shape=(9, 1), return_sequences=True, kernel_initializer='normal', activation=activation))
                model1.add(Dropout(dropout_rate))
                model1.add(LSTM(neurons, input_shape=(9, 1), return_sequences=False, kernel_initializer='normal', activation=activation))
                model1.add(Dense(1, kernel_initializer='normal'))
                model1.compile(loss='mean_squared_error', optimizer=opt)
                return model1
